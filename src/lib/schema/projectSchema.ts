...
export const ProjectMetricsSchema = z.object({
  usersReached: z
    ...
    .optional(),
  performanceImprovement: z
    .string()
    .max(
      200,
      'Performance improvement description must be less than 200 characters'
    )
    .trim()
    .optional(),
  revenueImpact: z
  ...
  uptime: z
    .string()
    .refine(val => !val || /^\d{1,3}(\.\d{1,2})?%?$/.test(val), {
      message: 'Uptime should be a percentage (e.g., 99.9% or 99.9)',
    })
    .optional(),
  loadTime: z
    .string()
    .refine(val => !val || /^\d+(\.\d+)?(ms|s)$/.test(val), {
...
export const ProjectSchema = z
  ...
  .refine(
    ...
    {
      message:
        'Featured projects should have detailed descriptions (at least 50 characters)',
      path: ['description'],
    }
  )
  .refine(
