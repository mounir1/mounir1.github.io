...
const futureDateSchema = z
  .union([
    ...
    z
      .string()
      .refine(val => !val || val === '' || new Date(val) > new Date(), {
...
const optionalStringArray = z
  .array(z.string().min(1, 'Item cannot be empty').trim())
  .default([]);
...
export const CertificationSchema = z.object({
  ...
  credentialId: z.string().max(100, 'Credential ID too long').trim().optional(),
  url: urlSchema,
...
export const LearningResourceSchema = z.object({
  ...
  provider: z.string().max(100, 'Provider name too long').trim().optional(),
  duration: z
    .string()
...
export const MilestoneSchema = z.object({
  title: z
    ...
    .trim(),
  description: z.string().max(300, 'Description too long').trim().optional(),
...
export const SkillExperienceSchema = z.object({
  ...
  months: z
    ...
    .default(0),
  totalMonths: z.number().min(0, 'Total months cannot be negative').optional(),
...
const SkillSchemaBase = z.object({
  ...
  level: z.union([z.nativeEnum(SkillLevel), z.number().min(0).max(100)]),
  ...
  proficiency: z
    ...
    .refine(val => val % 5 === 0 || val >= 90, {
      message:
        'Proficiency should be in increments of 5 (except for expert levels 90+)',
    })
    .optional()
    .default(50),
  ...
  projects: z
    ...
    .default([]),
  relatedSkills: z
    .array(z.string().min(1, 'Related skill ID cannot be empty'))
    .default([]),

  // Visual and Metadata
  icon: z.string().max(200, 'Icon path too long').trim().optional(),
  color: z
    .string()
...
export const SkillSchema = SkillSchemaBase.refine(
  ...
  .refine(
    ...
    {
      message:
        'Proficiency seems low for the experience level. Consider adjusting either value.',
      path: ['proficiency'],
    }
  )
  .refine(
  ...
  .refine(
    data => {
      ...
      if (data.proficiency && data.proficiency >= 80) {
        if (data.experience) {
          const totalMonths =
            data.experience.years * 12 + data.experience.months;
    ...
    {
      message:
        'High-proficiency skills should be backed by experience or certifications',
      path: ['experience'],
    }
  )
  .refine(
  ...
  .refine(
    data => {
      ...
      if (
        data.experience &&
        data.experience.firstUsed &&
        data.experience.lastUsed
      ) {
...
export const SkillUpdateSchema = SkillSchemaBase.partial()
  .omit({
    createdAt: true,
    schemaVersion: true,
  })
...
export const SkillCreateSchema = SkillSchemaBase.omit({
  ...
  createdAt: z
    .number()
    .positive('Created date must be valid')
    .default(() => Date.now()),
  updatedAt: z
    .number()
    .positive('Updated date must be valid')
    .default(() => Date.now()),
  version: z.number().int().min(1).default(1),
  schemaVersion: z.string().default('1.0.0'),
});
