...
export function optimizeData(
  ...
  const unusedIds = new Set(
    report.warnings.filter(w => w.type === 'unused_entity').map(w => w.entity)
  );
...
export function generateQualityReportString(report: DataQualityReport): string {
  ...
  if (report.errors.length > 0) {
    ...
    report.errors.forEach((error, i) => {
      lines.push(
        `  ${i + 1}. [${error.type}] ${error.entity}${error.field ? `.${error.field}` : ''}: ${error.message}`
      );
    });
  }
  ...
  if (report.warnings.length > 0) {
    ...
    report.warnings.forEach((warning, i) => {
      lines.push(
        `  ${i + 1}. [${warning.type}] ${warning.entity}: ${warning.message}`
      );
    });
  }
