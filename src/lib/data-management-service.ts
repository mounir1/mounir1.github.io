...
export class PortfolioDataManager {
  ...
  static async createProject(
    projectData: Omit<ProjectInput, 'id'>
  ): Promise<string> {
    try {
   ...
   */
  static validateProjectData(project: Partial<ProjectInput>): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      ...
      (project.priority < 0 || project.priority > 100)
    ) {
   ...
   */
  static validateSkillData(skill: Partial<SkillInput>): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      ...
      (skill.priority < 0 || skill.priority > 100)
    ) {
      errors.push('Priority must be between 0 and 100');
    }
