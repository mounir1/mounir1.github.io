          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span
                className={cn(
                  'text-5xl font-bold',
                  getQualityColor(stats.dataQualityScore)
                )}
              >
      ...
      {qualityReport && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-bold">
                  {qualityReport.stats.totalEntities}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-sm font-medium text-muted-foreground">
                ...
                <p
                  className={cn(
                    'text-2xl font-bold',
                    qualityReport.stats.duplicates > 0
                      ? 'text-orange-600'
                      : 'text-green-600'
                  )}
                >
              ...
              <div className="space-y-1">
                ...
                <p
                  className={cn(
                    'text-2xl font-bold',
                    qualityReport.stats.brokenReferences > 0
                      ? 'text-red-600'
                      : 'text-green-600'
                  )}
                >
              ...
              <div className="space-y-1">
                ...
                <p
                  className={cn(
                    'text-2xl font-bold',
                    qualityReport.stats.unusedEntities > 0
                      ? 'text-yellow-600'
                      : 'text-green-600'
                  )}
                >
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {qualityReport.errors.map((error, index) => (
                    <div
                      key={index}
                      className="flex items-start gap-2 rounded-md border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-950/20"
                    >
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-64 space-y-2 overflow-y-auto">
                  {qualityReport.warnings.map((warning, index) => (
                    <div
                      key={index}
                      className="flex items-start gap-2 rounded-md border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-950/20"
                    >
            ...
            {qualityReport.errors.length === 0 &&
              qualityReport.warnings.length === 0 && (
                <div className="flex items-center gap-2 rounded-md border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-950/20">
                  <CheckCircle2 className="h-5 w-5 text-green-600" />
                  <p className="text-sm font-medium text-green-900 dark:text-green-100">
                    All data quality checks passed! Your data is in excellent
                    condition.
                  </p>
                </div>
              )}
