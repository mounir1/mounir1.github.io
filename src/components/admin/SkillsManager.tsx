...
export function SkillsManager() {
  const { skills, loading, addSkill, updateSkill, deleteSkill, refetch } =
    useSkills();
  ...
  const onSubmit = async (values: SkillInput) => {
    try {
      ...
      if (skillToEdit) {
        ...
        if (
          skillToEdit.id.startsWith('local-') ||
          skillToEdit.id.startsWith('fallback-')
        ) {
          toast({
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes('permission')) {
          errorMessage =
            "You don't have permission to modify skills. Please check your access rights.";
  ...
  const handleEditSkill = useCallback(
    (skill: Skill) => {
      ...
      form.reset({
        ...
        yearsOfExperience:
          skill.yearsOfExperience || skill.experience?.years || 0,
  ...
  const confirmDeleteSkill = useCallback(async () => {
    ...
    try {
      ...
      if (
        skillToDelete.id.startsWith('local-') ||
        skillToDelete.id.startsWith('fallback-')
      ) {
        toast({
  ...
  const confirmBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      const deletableSkills = skillsToDelete.filter(
        skill =>
          !skill.id.startsWith('local-') && !skill.id.startsWith('fallback-')
      );
  ...
  const columns: ColumnDef<Skill>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            />
            <span className="w-8 text-sm text-muted-foreground">{val}%</span>
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        const years =
          row.original.yearsOfExperience || row.original.experience?.years || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto bg-background text-foreground">
          <DialogHeader>
            <DialogTitle>
              {skillToEdit ? 'Edit Skill' : 'Add Skill'}
            </DialogTitle>
            <DialogDescription>
              {skillToEdit
              ...
              <ScrollArea className="max-h-[60vh] pr-4">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <FormField
                    ...
                    name="featured"
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border border-input bg-background p-4 text-foreground">
                        <FormControl>
                          <Checkbox
                            checked={field.value}
        ...
        <AlertDialogContent className="bg-background text-foreground">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {skillToDelete?.id.startsWith('local-') ||
              skillToDelete?.id.startsWith('fallback-') ? (
                <span className="mt-2 block font-medium text-yellow-500">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="border-input bg-background text-foreground hover:bg-accent hover:text-accent-foreground">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
        ...
        <AlertDialogContent className="bg-background text-foreground">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{skillsToDelete.length}</strong> skills.
              {skillsToDelete.some(
                s => s.id.startsWith('local-') || s.id.startsWith('fallback-')
              ) ? (
                <span className="mt-2 block font-medium text-yellow-500">
