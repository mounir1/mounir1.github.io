...
### [x] Resume: regenerated from verified data — session 7

`public/Mounir_Abderrahmani_Resume.pdf` is now generated by
`scripts/generate_cv.py` directly from the fact-checked data files
(`initial-experience.ts`, `initial-projects.ts`, `initial-skills.ts`),
replacing the earlier uploaded PDF that contained unverifiable metrics
(100K+ txn/month, 100+ hotels, 5TB+, 99.9% uptime). Adds the
repo-verified WebEX engagement (2017–present) and corrected JSKit
identity. The stale duplicate `/Mounir_CV_2025.pdf` was removed; the
script writes straight to the served filename. To regenerate after
data changes: `python3 scripts/generate_cv.py`.
